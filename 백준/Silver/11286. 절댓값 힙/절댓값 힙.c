...
void MinHeapIfyUp(MinHeap * heap, int idx) {
  while(idx > 1 && compareAbs(heap -> arr[idx], heap -> arr[idx/2])) {
...
void MinHeapIfyDown(MinHeap * heap, int idx) {
  ...
  if (left_child_idx <= heap -> size && compareAbs(heap -> arr[left_child_idx], heap -> arr[smallest])) {
    smallest = left_child_idx;
  }

  if (right_child_idx <= heap -> size && compareAbs(heap -> arr[right_child_idx], heap -> arr[smallest])) {
